"""Serviços da aplicação para gerenciamento e montagem de releases."""
...
def preview_split_metrics(
    ...
    revisions = list_annotation_revisions(ws, campaign_id)
    ...
    report = load_annotation_revision_report(ws, campaign_id, target_rev)
...
def training_recipe(ws: Workspace, release_id: str, params: TrainingParams | None = None) -> dict[str, Any]:
    root = release_root(ws, release_id)
    data_yaml = root / "data.yaml"
    if not data_yaml.exists():
        raise WorkflowError("Materialize a release antes de configurar o treinamento.")

    p = params or TrainingParams(project=str(ws.root / "runs" / "detect"), name=release_id)
...
def release_status(ws: Workspace, release_id: str) -> dict[str, Any]:
    config = load_yaml(release_config_path(ws, release_id))
    root = release_root(ws, release_id)
    ...
    data_yaml = root / "data.yaml"
    recipe = training_recipe(ws, release_id) if data_yaml.exists() else None
    return {
        "release_id": release_id,
