"""Regras de domínio e ciclo de vida de campanhas de dataset."""
...
def campaigns_root(defaults_or_ws: dict[str, Any] | Workspace) -> Path:
    if isinstance(defaults_or_ws, Workspace):
        return defaults_or_ws.campaigns_root
    p = Path(defaults_or_ws["paths"]["campaigns_root"])
    return p if p.is_absolute() else p.resolve()


def campaign_root(defaults_or_ws: dict[str, Any] | Workspace, campaign_id: str) -> Path:
    return campaigns_root(defaults_or_ws) / campaign_id


def campaign_config_path(defaults_or_ws: dict[str, Any] | Workspace, campaign_id: str) -> Path:
    return campaign_root(defaults_or_ws, campaign_id) / "campaign.yaml"


def list_campaigns(defaults_or_ws: dict[str, Any] | Workspace) -> list[str]:
    root = campaigns_root(defaults_or_ws)
    if not root.exists():
        return []
    return sorted(
        path.name for path in root.iterdir() if (path / "campaign.yaml").is_file()
    )
...
def create_campaign(
    ...
    *,
    ...
    if isinstance(defaults_or_ws, Workspace):
        ...
        ws = Workspace.from_path(Path(defaults["paths"]["campaigns_root"]).parent.parent)

    root = campaign_root(defaults, campaign_id)
    if root.exists():
        raise WorkflowError(f"A campanha ja existe: {root}")
    ...
    if mode not in {"smart", "uniform"}:
        raise WorkflowError("A campanha aceita extraction.mode smart ou uniform.")
    if mode == "uniform":
    ...
    if annotation_backend not in {"none", "local"}:
        raise WorkflowError(
            "A campanha aceita annotation.backend none ou local."
        )
...
def load_campaign(defaults_or_ws: dict[str, Any] | Workspace, campaign_id: str) -> dict[str, Any]:
    payload = load_yaml(campaign_config_path(defaults_or_ws, campaign_id))
    if payload.get("campaign_id") != campaign_id:
        raise WorkflowError("campaign.yaml possui identificador divergente.")
    return payload


def frame_manifest_path(defaults_or_ws: dict[str, Any] | Workspace, campaign_id: str) -> Path:
    return campaign_root(defaults_or_ws, campaign_id) / "frame_manifest.json"


def import_tasks_path(defaults_or_ws: dict[str, Any] | Workspace, campaign_id: str) -> Path:
    return campaign_root(defaults_or_ws, campaign_id) / "label_studio" / "import_tasks.json"


def selected_export_path(defaults_or_ws: dict[str, Any] | Workspace, campaign_id: str) -> Path:
    return campaign_root(defaults_or_ws, campaign_id) / "label_studio" / "export_selected.json"


def save_selected_export(
    defaults_or_ws: dict[str, Any] | Workspace, campaign_id: str, content: bytes
) -> Path:
    load_campaign(defaults_or_ws, campaign_id)
    try:
    ...
    destination = selected_export_path(defaults_or_ws, campaign_id)
...
def export_annotations_path(defaults_or_ws: dict[str, Any] | Workspace, campaign_id: str) -> Path:
    return campaign_root(defaults_or_ws, campaign_id) / "label_studio" / "export_annotations.json"


def annotation_report_path(defaults_or_ws: dict[str, Any] | Workspace, campaign_id: str) -> Path:
    return campaign_root(defaults_or_ws, campaign_id) / "label_studio" / "annotation_report.json"


def annotation_revisions_root(defaults_or_ws: dict[str, Any] | Workspace, campaign_id: str) -> Path:
    return campaign_root(defaults_or_ws, campaign_id) / "label_studio" / "revisions"


def annotation_revision_root(
    defaults_or_ws: dict[str, Any] | Workspace, campaign_id: str, revision_id: str
) -> Path:
    validate_id(revision_id, "revision_id")
    return annotation_revisions_root(defaults_or_ws, campaign_id) / revision_id
...
def annotation_revision_export_path(
    ...
    return (
        annotation_revision_root(defaults_or_ws, campaign_id, revision_id)
...
def list_annotation_revisions(
    defaults_or_ws: dict[str, Any] | Workspace, campaign_id: str
) -> list[str]:
    root = annotation_revisions_root(defaults_or_ws, campaign_id)
    revisions = (
        sorted(
            path.name
    ...
    if (
        export_annotations_path(defaults_or_ws, campaign_id).exists()
        and annotation_report_path(defaults_or_ws, campaign_id).exists()
    ):
...
def annotation_source_paths(
    ...
    if revision_id == "legacy":
        return (
            export_annotations_path(defaults_or_ws, campaign_id),
            annotation_report_path(defaults_or_ws, campaign_id),
        )
    return (
        annotation_revision_export_path(defaults_or_ws, campaign_id, revision_id),
        annotation_revision_report_path(defaults_or_ws, campaign_id, revision_id),
    )
...
def load_annotation_revision_report(
    ...
    if not export_path.exists() or not report_path.exists():
        raise WorkflowError(
            f"Revisao de anotacao nao encontrada: {campaign_id}/{revision_id}"
        )
...
def load_frame_manifest(defaults_or_ws: dict[str, Any] | Workspace, campaign_id: str) -> dict[str, Any]:
    path = frame_manifest_path(defaults_or_ws, campaign_id)
    if not path.exists():
...
def build_import_tasks(defaults_or_ws: dict[str, Any] | Workspace, campaign_id: str) -> Path:
    campaign = load_campaign(defaults_or_ws, campaign_id)
    manifest = load_frame_manifest(defaults_or_ws, campaign_id)
    root = campaign_root(defaults_or_ws, campaign_id)
    dataset_root = campaigns_root(defaults_or_ws).parent
    ...
    for frame in manifest["frames"]:
        ...
        tasks.append(task)
    output = import_tasks_path(defaults_or_ws, campaign_id)
